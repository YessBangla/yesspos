...
export const Route = createFileRoute("/_authenticated/data-backup")({
  head: () => ({
    meta: [
      { title: "Daily data backup & restore — Sokoler Bazar" },
      { name: "description", content: "Download every day's shop data as a backup file and upload it again to restore." },
      { property: "og:title", content: "Daily data backup & restore — Sokoler Bazar" },
...
function DataBackupPage() {
  ...
  async function handleDownload() {
    ...
    try {
      ...
      downloadJson(`sokoler-bazar-backup-${full ? "full" : from === to ? from : `${from}_${to}`}.json`, file);
      downloadCsv(
        `sokoler-bazar-backup-summary-${from}.csv`,
        [t("table"), t("rows")],
